...
InputParameter::parse_cfg(const string & in_file)
{
  ...
  while(fscanf(fp, "%[^\n]\n", line) != EOF) {
   ...
   //yash
   ////including adder_type_step1 into config file
    if (!strncmp("-Adder type", line, strlen("-Adder type"))) {
      sscanf(line, "-Adder type %[^\"]\"%[^\"]\"", jk, temp_var);

      if(!strncmp("cram", temp_var, strlen("cram"))) {
        adder_type = 0;
      }
      else if(!strncmp("new", temp_var, strlen("novel"))) {
        adder_type = 1;
      }
      else if(!strncmp("blade", temp_var, strlen("blade"))) {
        adder_type = 2;
      }
      else if(!strncmp("iscas", temp_var, strlen("iscas"))) {
        adder_type = 3;
      }
      else if(!strncmp("dac", temp_var, strlen("dac"))) {
        adder_type = 4;
      }
      // else if(!strncmp("new", temp_var, strlen("new"))) {
      //   adder_type = 5;
      // }
      else {
    ...
    //yash
    //including senseamptype_
    if (!strncmp("-Sense AMP", line, strlen("-Sense AMP"))) {
      sscanf(line, "-Sense AMP %[^\"]\"%[^\"]\"", jk, temp_var);

      if(!strncmp("CONV", temp_var, strlen("CONV"))) {
        sense_amp_type = 0;
      }
      else if(!strncmp("MRSA", temp_var, strlen("MRSA"))) {
        sense_amp_type = 1;
      }
      else if(!strncmp("CESA", temp_var, strlen("CESA"))) {
        sense_amp_type = 2;
      }
      else if(!strncmp("NORMAL", temp_var, strlen("NORMAL"))) {
        sense_amp_type = 3;
    ...
    //cout<<"sense_amp_type :"<<sense_amp_type<<endl;



    //yash
    //including adder_type_step6
    if (!strncmp("-Operation type", line, strlen("-Operation type"))) {
      sscanf(line, "-Operation type %[^\"]\"%[^\"]\"", jk, temp_var);

      if(!strncmp("bit_serial", temp_var, strlen("bit_serial"))) {
        operation_type = 0;
      }
      else if(!strncmp("bit_parallel", temp_var, strlen("bit_parallel"))) {
        operation_type = 1;
      }
      else {
        cout << "ERROR: Invalid type!\n";
    ...
    //yash including number of bits operations

    if (!strncmp("-Nbits", line, strlen("-Nbits"))) {
      sscanf(line, "-Nbits %d", &(n_bit));
      continue;
    }

    if (!strncmp("-Extract_Output", line, strlen("-Extract_Output"))) {
      sscanf(line, "-Extract_Output %d", &(extract_output));
      continue;
    }
...
uca_org_t cacti_interface(const string & infile_name)
{
  ...
  uca_org_t fin_res;//final result yash
//  uca_org_t result;
  fin_res.valid = false;
...
void output_UCA(uca_org_t *fr)
{
  ...
  cout << "    No. of Columns in Subarray : " << fr->data_array2->no_of_columns << endl;//yash no. of columns added to print
  cout << "    No. of Rows in Subarray : " << fr->data_array2->no_of_rows << endl;//yash no. of rows added to print
  ...
  if (g_ip->print_detail)
  {
    ...
    cout <<  "\tDecoder(ns): " <<
      fr->data_array2->delay_row_predecode_driver_and_block * 1e9 << endl;

    cout <<  "\twordline delay data (ns): " <<
      fr->data_array2->delay_row_decoder * 1e9 << endl;

    cout <<  "\tBitline delay data (ns): " <<
      fr->data_array2->delay_bitlines/1e-9 << endl;

    cout <<  "\tSense Amplifier delay data(ns): " <<
      fr->data_array2->delay_sense_amp * 1e9 << endl;
    
    // cout <<  "\tCompute (ns): " <<
    //   fr->data_array2->compute_adder_delay * 1e9 << endl; //adder_type_step17 yash----->transfered to Compute_adder_delay
    ...
    cout <<endl<< "__debgug parameters _____"<<endl;
    cout << "\tBitline_cap(fF): "<<fr->data_array2->bitline_capacitance * 1e15<<endl;
    cout << "\tNumber of subarrays per mat: "<<fr->data_array2->number_of_subarrays_per_mat<<endl;
    cout << "\tSingle_multiplier_energy (nJ): " <<
      fr->data_array2->single_multiplier_energy.computeOp.dynamic * 1e9 << endl;
    cout << "\tSingle_adder_energy (nJ): " <<
      fr->data_array2->single_adder_energy.computeOp.dynamic * 1e9 << endl;
    
    cout << endl << "____Computation delay Components:______" << endl << endl;
    cout << "\t______" << g_ip->n_bit << "_______bit computation"  <<endl;
    cout << "Bit computation" << g_ip->n_bit <<endl;
    cout <<  "\taccess_time_upto_sense_amp_out (ns): " <<
      fr->data_array2->start_to_sense_delay * 1e9 << endl;
    cout <<  "\tCompute_adder_delay (ns): " <<
      fr->data_array2->compute_adder_delay * 1e9 << endl;
    cout <<  "\tCompute_mutliplier_delay (ns): " <<
      fr->data_array2->compute_multiplier_delay * 1e9 << endl;
    cout <<  "\tWrite_back_delay (ns): " <<
      fr->data_array2->write_back_delay * 1e9 << endl;

    cout << endl<< "IMC_CYCLE_TIME (ns): "<< fr->data_array2->cycle_time_imc *1e9 <<endl;
    cout << endl<< "IMC_CYCLE_TIME_ADD (ns): "<<fr->data_array2->cycle_time_imc_add*1e9<<endl;

    if (g_ip->operation_type == 0)
    {
      cout << endl<<  endl<< "__Bit serial Addition__"<<endl;
      //addition delay, per compute delay,edp,per compute edp
       cout << endl<<  "\tBit_serial_delay_for_Nbits_addtion (ns): " <<
      fr->data_array2->total_nbit_serial_delay_add * 1e9 << endl; // add delay

      cout <<  "\tBit_serial_energy_for_Nbits_addtion (nJ): " <<
      fr->data_array2->Total_energy_bit_serial_add.computeOp.dynamic * 1e9 << endl; //add energy

      cout <<  "\tBit_serial_edp_for_Nbits_addtion (ns*nJ): " <<
      fr->data_array2->total_nbit_serial_delay_add * 1e9 * 
      fr->data_array2->Total_energy_bit_serial_add.computeOp.dynamic * 1e9<< endl; //edp add

      cout <<  "\tBit_serial_edp_per_compute_Nbits_addtion (ns*nJ): " <<
      fr->data_array2->total_nbit_serial_delay_add * 1e9 * (fr->data_array2->
      Total_energy_bit_serial_add.computeOp.dynamic/fr->data_array2->
      num_parallel_computes_bs_subarray) * 1e9 << endl<<endl; //edp add per compute

      cout <<   "\tBit_serial_TOPS_for_Nbits_addtion (TOPS): " <<
      (fr->data_array2-> num_parallel_computes_bs_subarray/ 
      (fr->data_array2->total_nbit_serial_delay_add* 1e12))  << endl; //TOPS= no. of parallel computes/add delay

      // cout <<  "\tBit_serial_TOPS/W/Computes_for_Nbits_addtion (TOPS/Watt/Compute): " <<
      // (1/(fr->data_array2->Total_energy_bit_serial_add.computeOp.dynamic * 1e12)) << endl; //(TOPS/W/Computes = 1/add energy)
      cout <<  "\tBit_serial_TOPS/W/Computes_for_Nbits_addtion (TOPS/Watt/Compute): " <<
      (fr->data_array2->num_parallel_computes_bs_subarray/
      (fr->data_array2->Total_energy_bit_serial_add.computeOp.dynamic * 1e12)) << endl; //(TOPS/W/Computes = 1/add energy per compute)



      cout << endl<<  endl<< "__Bit serial Multiplication__"<<endl;
      //mutliplication delay,edp,per compute edp
      cout <<  "\tBit_serial_delay_for_Nbits_multiplication (ns): " <<
      fr->data_array2->total_nbit_serial_delay_multiplier * 1e9 << endl; //mult delay

      cout <<  "\tBit_serial_energy_for_Nbits_multiplication (nJ): " <<
      fr->data_array2->Total_energy_bit_serial_multiplication.computeOp.dynamic * 1e9 << endl<<endl;//mult energy      

      cout <<  "\tBit_serial_edp_for_Nbits_multiplication (ns*nJ): " <<
      fr->data_array2->total_nbit_serial_delay_multiplier * 1e9 * 
      fr->data_array2->Total_energy_bit_serial_multiplication.computeOp.dynamic * 1e9 << endl; // mult edp

      cout <<  "\tBit_serial_edp_per_compute_Nbits_multiplication (ns*nJ): " <<
      fr->data_array2->total_nbit_serial_delay_multiplier * 1e9 * (fr->data_array2->
      Total_energy_bit_serial_multiplication.computeOp.dynamic/fr->data_array2->
      num_parallel_computes_bs_subarray) * 1e9 << endl<<endl;      //mult edp per compute

      cout <<  "\tBit_serial_TOPS_for_Nbits_multiplication (TOPS): " <<
      fr->data_array2->num_parallel_computes_bs_subarray/(fr->
      data_array2->total_nbit_serial_delay_multiplier * 1e12) << endl; //TOPS= no. of parallel computes/mult delay

      // cout <<  "\tBit_serial_TOPS/W/Computes_for_Nbits_multiplication (TOPS/Watt/Compute): " <<
      // (1/(fr->data_array2->Total_energy_bit_serial_multiplication.computeOp.dynamic * 1e12)) << endl<<endl; //(TOPS/W/Computes = 1/mult_energy)

      cout <<  "\tBit_serial_TOPS/W/Computes_for_Nbits_multiplication (TOPS/Watt/Compute): " <<
      (fr->data_array2->num_parallel_computes_bs_subarray
      /(fr->data_array2->Total_energy_bit_serial_multiplication.computeOp.dynamic * 1e12)) << endl<<endl; //(TOPS/W/Computes = 1/mult_energy per compute)

      std::string adder_type_string;
      if(g_ip->adder_type == 0){
        adder_type_string = "CRAM";
      }
      else if(g_ip->adder_type == 1){
        adder_type_string = "NEW";
      }
      else if(g_ip->adder_type == 2){
        adder_type_string = "BLADE";
      }
      else if(g_ip->adder_type == 3){
        adder_type_string = "ISCAS";
      }
      else if(g_ip->adder_type == 4){
        adder_type_string = "DAC";
      }

      std::string sense_amp_string;
      if(g_ip->sense_amp_type == 0){
        sense_amp_string = "CONV";
      }
      else if(g_ip->sense_amp_type == 1){
        sense_amp_string = "MRSA";
      }
      else if(g_ip->sense_amp_type == 2){
        sense_amp_string = "CESA";
      }

      if(g_ip->extract_output==1){
        std::string fileName = "Output_Himanshu.txt";
        // std::string data = "Bit Serial,"
        // +std::to_string(static_cast<int>(fr->data_array2->no_of_rows))+","
        // +std::to_string(static_cast<int>(fr->data_array2->no_of_columns))+","
        // +sense_amp_string+","
        // +std::to_string(fr->data_array2->total_nbit_serial_delay_add * 1e9)+","
        // +std::to_string(fr->data_array2->Total_energy_bit_serial_add.computeOp.dynamic * 1e9)+","
        // +std::to_string(fr->data_array2->num_parallel_computes_bs_subarray/(fr->data_array2->total_nbit_serial_delay_add* 1e12))+","
        // +std::to_string(fr->data_array2->num_parallel_computes_bs_subarray/(fr->data_array2->Total_energy_bit_serial_add.computeOp.dynamic * 1e12))+","
        // +std::to_string(fr->data_array2->total_nbit_serial_delay_multiplier * 1e9)+","
        // +std::to_string(fr->data_array2->Total_energy_bit_serial_multiplication.computeOp.dynamic * 1e9)+","
        // +std::to_string(fr->data_array2->num_parallel_computes_bs_subarray/(fr->data_array2->total_nbit_serial_delay_multiplier * 1e12))+","
        // +std::to_string(fr->data_array2->num_parallel_computes_bs_subarray/(fr->data_array2->Total_energy_bit_serial_multiplication.computeOp.dynamic * 1e12));
        ...
        +adder_type_string+","
        //addition delays
        +std::to_string(fr->data_array2->delay_precharge * 1e9)+","
        +std::to_string(fr->data_array2->delay_predec * 1e9)+","
        +std::to_string(fr->data_array2->delay_Wordline_R * 1e9)+","
        +std::to_string(fr->data_array2->delay_Bitline_R * 1e9)+","
        +std::to_string(fr->data_array2->delay_BitMux_C * 1e9)+","
        +std::to_string(fr->data_array2->delay_SA_R_C * 1e9)+","
        +std::to_string(fr->data_array2->delay_Add * 1e9)+","
        +std::to_string(fr->data_array2->delay_Mult * 1e9)+","
        +std::to_string(fr->data_array2->delay_WriteDriver * 1e9)+","
        +std::to_string(fr->data_array2->delay_Wordline_R * 1e9)+","
        +std::to_string(fr->data_array2->cycle_time_imc_add * 1e9)+","
        +std::to_string(fr->data_array2->cycle_time_imc * 1e9)+","
        +std::to_string(fr->data_array2->total_nbit_serial_delay_add * 1e9)+","
        +std::to_string(fr->data_array2->total_nbit_serial_delay_multiplier * 1e9)+","
        //addition delays complete
        //addition energy
        +std::to_string(fr->data_array2->power_prechg_eq_drivers.readOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->power_rowdec_for_compute.computeOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->power_bitmux_for_compute.computeOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->power_bitline_for_compute.computeOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->power_sa_for_compute.computeOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->Power_SA_mux_lv1_dec.computeOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->Power_SA_mux_lv2_dec.computeOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->energy_per_compute_add.computeOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->energy_per_compute_mult.computeOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->power_rowdec_for_writeback.writebackOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->power_bitmux_for_compute.computeOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->energy_for_one_compuatation_add.computeOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->energy_for_one_compuatation_multiply.computeOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->Total_energy_bit_serial_add.computeOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->Total_energy_bit_serial_multiplication.computeOp.dynamic * 1e9)+","
        //addition energy complete
        //TOPS and TOPS/watt/compute
        +std::to_string(fr->data_array2->num_parallel_computes_bs_subarray/(fr->data_array2->total_nbit_serial_delay_add* 1e12))+","
        +std::to_string(fr->data_array2->num_parallel_computes_bs_subarray/(fr->data_array2->Total_energy_bit_serial_add.computeOp.dynamic * 1e12))+","
        +std::to_string(fr->data_array2->num_parallel_computes_bs_subarray/(fr->data_array2->total_nbit_serial_delay_multiplier * 1e12))+","
        +std::to_string(fr->data_array2->num_parallel_computes_bs_subarray/(fr->data_array2->Total_energy_bit_serial_multiplication.computeOp.dynamic * 1e12));
        // +std::to_string(fr->data_array2->energy_per_access.computeOp.dynamic * 1e9)+","
        // +std::to_string(fr->data_array2->energy_per_compute_add.computeOp.dynamic * 1e9)+","
        // +std::to_string(fr->data_array2->energy_per_compute_mult.computeOp.dynamic * 1e9)+","
        // +std::to_string(fr->data_array2->energy_per_writeback.computeOp.dynamic * 1e9);

        // Write data to the file
        writeDataToFile(fileName, data);

        std::cout << "Data appended to the file successfully." << std::endl;
        std::cout << "Test variable value: "<< fr->data_array2->test << std::endl;
    }

    }
    else if (g_ip->operation_type == 1)
    {
        cout << endl<<  endl<< "__Bit parallel Addition__"<<endl;
      //addition delay,edp,per compute edp
       cout << endl<< "\tBit_parallel_delay_for_Nbits_addition (ns): " <<
      fr->data_array2->total_nbit_parallel_delay_add * 1e9 << endl; //add delay

      cout <<  "\tBit_parallel_energy_for_Nbits_addtion (nJ): " <<
      fr->data_array2->Total_energy_bit_parallel_add.computeOp.dynamic * 1e9 << endl; //add energy

      cout <<  "\tBit_parallel_edp_for_Nbits_addition (ns*nJ): " <<
      fr->data_array2->total_nbit_parallel_delay_add * 1e9 * 
      fr->data_array2->Total_energy_bit_parallel_add.computeOp.dynamic * 1e9<< endl; //add edp

      cout <<  "\tBit_parallel_edp_per_compute_Nbits_addition (ns*nJ): " <<
      fr->data_array2->total_nbit_parallel_delay_add * 1e9 * (fr->data_array2->
      Total_energy_bit_parallel_add.computeOp.dynamic/fr->data_array2->
      num_parallel_computes_bp_subarray) * 1e9 << endl<<endl;        // add edp per compute

      cout << endl<< "\tBit_parallel_TOPS_for_Nbits_addition (TOPS): " <<
      fr->data_array2->num_parallel_computes_bp_subarray/(fr->data_array2->
      total_nbit_parallel_delay_add * 1e12) << endl; //TOPS= (no. of parallel computes/add delay)

      // cout <<  "\tBit_parallel_TOPS/Watt/Computes_for_Nbits_addtion (TOPS/Watt/Computes): " <<
      // 1/(fr->data_array2->Total_energy_bit_parallel_add.computeOp.dynamic * 1e12) << endl; //TOPS/Watt/Computes = 1/add energy

      cout <<  "\tBit_parallel_TOPS/Watt/Computes_for_Nbits_addtion (TOPS/Watt/Computes): " <<
      (fr->data_array2->num_parallel_computes_bp_subarray/
      (fr->data_array2->Total_energy_bit_parallel_add.computeOp.dynamic * 1e12)) << endl; //TOPS/Watt/Computes = 1/add energy per compute


      cout << endl<<  endl<< "__Bit parallel Multiplication__"<<endl;
      //multiplication delay,edp,per compute edp
      cout <<  "\tBit_parallel_delay_for_Nbits_multiplication (ns): " <<
      fr->data_array2->total_nbit_parallel_delay_multiplier * 1e9  << endl; //mult delay

      cout <<  "\tBit_parallel_energy_for_Nbits_multiplication (nJ): " <<
      fr->data_array2->Total_energy_bit_parallel_multiplier.computeOp.dynamic * 1e9 << endl;//mult energy          

      cout <<  "\tBit_parallel_edp_for_Nbits_multiplication (ns*nJ): " <<
      fr->data_array2->total_nbit_parallel_delay_multiplier * 1e9 * 
      fr->data_array2->Total_energy_bit_parallel_multiplier.computeOp.dynamic * 1e9 << endl;//mult edp
      
      cout <<  "\tBit_parallel_edp_per_compute_Nbits_multiplication (ns*nJ): " <<
      fr->data_array2->total_nbit_parallel_delay_multiplier * 1e9 * (fr->data_array2->
      Total_energy_bit_parallel_multiplier.computeOp.dynamic/fr->data_array2->
      num_parallel_computes_bp_subarray) * 1e9 << endl<<endl;     //mult edp per compute   

      cout <<  "\tBit_parallel_TOPS_Nbits_multiplication (TOPS): " <<
      (fr->data_array2-> num_parallel_computes_bp_subarray/
      (fr->data_array2->total_nbit_parallel_delay_multiplier * 1e12))  << endl; // TOPS = no. of parallel computes/mult delay

      // cout <<  "\tBit_parallel_TOPS/Watt/Computes_for_Nbits_multiplication (TOPS/Watt/Computes): " <<
      // (1/(fr->data_array2->Total_energy_bit_parallel_multiplier.computeOp.dynamic * 1e12)) << endl; 
      // //TOPS/watt/computes = 1/mult energy

      cout <<  "\tBit_parallel_TOPS/Watt/Computes_for_Nbits_multiplication (TOPS/Watt/Computes): " <<
      (fr->data_array2->num_parallel_computes_bp_subarray
      /(fr->data_array2->Total_energy_bit_parallel_multiplier.computeOp.dynamic * 1e12)) << endl; 
      //TOPS/watt/computes = 1/mult energy per compute
      ...
      if(g_ip->extract_output==1){
        std::string fileName = "Output_Himanshu.txt";
        // std::string data = "Bit Parallel,"
        // +std::to_string(static_cast<int>(fr->data_array2->no_of_rows))+","
        // +std::to_string(static_cast<int>(fr->data_array2->no_of_columns))+","
        // +sense_amp_string+","
        // +std::to_string(fr->data_array2->total_nbit_parallel_delay_add * 1e9)+","
        // +std::to_string(fr->data_array2->Total_energy_bit_parallel_add.computeOp.dynamic * 1e9)+","
        // +std::to_string(fr->data_array2->num_parallel_computes_bp_subarray/(fr->data_array2->total_nbit_parallel_delay_add * 1e12))+","
        // +std::to_string(fr->data_array2->num_parallel_computes_bp_subarray/(fr->data_array2->Total_energy_bit_parallel_add.computeOp.dynamic * 1e12))+","
        // +std::to_string(fr->data_array2->total_nbit_parallel_delay_multiplier * 1e9)+","
        // +std::to_string(fr->data_array2->Total_energy_bit_parallel_multiplier.computeOp.dynamic * 1e9)+","
        // +std::to_string(fr->data_array2-> num_parallel_computes_bp_subarray/(fr->data_array2->total_nbit_parallel_delay_multiplier * 1e12))+","
        // +std::to_string(fr->data_array2->num_parallel_computes_bp_subarray/(fr->data_array2->Total_energy_bit_parallel_multiplier.computeOp.dynamic * 1e12));
        // Replace this with the data you want to add
        ...
        +std::to_string(fr->data_array2->power_bitmux_for_compute.computeOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->Total_energy_bit_parallel_add.computeOp.dynamic * 1e9)+","
        +std::to_string((fr->data_array2->Total_energy_bit_parallel_multiplier.computeOp.dynamic/(g_ip->n_bit + 4)) * 1e9)+","
        +std::to_string(fr->data_array2->Total_energy_bit_parallel_add.computeOp.dynamic * 1e9)+","
        +std::to_string(fr->data_array2->Total_energy_bit_parallel_multiplier.computeOp.dynamic * 1e9)+","
        //addition energy complete
        //TOPS and TOPS/watt/compute
    ...
    if (g_ip->operation_type == 0)
    {
       ...
       cout << endl<<  endl;
      //per compute energy add  & mult
      cout <<  "\tBit_serial_energy_per_compute_Nbits_addtion (nJ): " <<
      (fr->data_array2->Total_energy_bit_serial_add.computeOp.dynamic/fr->data_array2->
      num_parallel_computes_bs_subarray) * 1e9 << endl;

      cout <<  "\tBit_serial_energy_per_compute_Nbits_multiplication (nJ): " <<
      (fr->data_array2->Total_energy_bit_serial_multiplication.computeOp.dynamic/fr->data_array2->
      num_parallel_computes_bs_subarray) * 1e9 << endl<<endl;

      cout <<  "\tBit_serial_delay_Wordline (ns): " <<
      (fr->data_array2->delay_Wordline_R) * 1e9 << endl;
      cout <<  "\tBit_serial_delay_Bitline (ns): " <<
      (fr->data_array2->delay_Bitline_R) * 1e9 << endl;
      cout <<  "\tBit_serial_delay_SA (ns): " <<
      (fr->data_array2->delay_SA_R_C) * 1e9 << endl;
      cout <<  "\tBit_serial_delay_Bitline_MUX (ns): " <<
      (fr->data_array2->delay_BitMux_C) * 1e9 << endl;
      cout <<  "\tBit_serial_delay_MAX_RC (ns): " <<
      (fr->data_array2->delay_R_C_max) * 1e9 << endl;
      cout <<  "\tBit_serial_delay_Adder (ns): " <<
      (fr->data_array2->delay_Add) * 1e9 << endl;
      cout <<  "\tBit_serial_delay_Multiplier (ns): " <<
      (fr->data_array2->delay_Mult) * 1e9 << endl;
      cout <<  "\tBit_serial_delay_WriteDriver (ns): " <<
      (fr->data_array2->delay_WriteDriver) * 1e9 << endl;
      cout <<  "\tBit_serial_delay_MAX_AddWdBit_Wordline (ns): " <<
      (fr->data_array2->delay_AWdB_W_max) * 1e9 << endl;
      cout <<  "\tBit_serial_delay_MAX_MultWdBit_Wordline (ns): " <<
      (fr->data_array2->delay_MWdB_W_max) * 1e9 << endl;
      cout <<  "\tBit_serial_delay_Final_A_single (ns): " <<
      (fr->data_array2->delay_final_A) * 1e9 << endl;
      cout <<  "\tBit_serial_delay_Final_M_single (ns): " <<
      (fr->data_array2->delay_final_M) * 1e9 << endl;
      cout <<  "\tBit_serial_delay_Final_ADD (ns): " <<
      (fr->data_array2->test) * 1e20 << endl;
    ...
    else if (g_ip->operation_type == 1)
    {
      cout << endl<<  endl;
      //per compute //energy add  & mult
      cout <<  "\tBit_parallel_energy_per_compute_Nbits_addtion (nJ): " <<
      (fr->data_array2->Total_energy_bit_parallel_add.computeOp.dynamic/fr->data_array2->
      num_parallel_computes_bp_subarray) * 1e9 << endl;
      
      cout <<  "\tBit_parallel_energy_per_compute_Nbits_multiplication (nJ): " <<
      (fr->data_array2->Total_energy_bit_parallel_multiplier.computeOp.dynamic/fr->data_array2->
      num_parallel_computes_bp_subarray) * 1e9 << endl;

      cout <<  "\tBit_parallel_delay_Wordline (ns): " <<
      (fr->data_array2->delay_Wordline_R) * 1e9 << endl;
      cout <<  "\tBit_parallel_delay_Bitline (ns): " <<
      (fr->data_array2->delay_Bitline_R) * 1e9 << endl;
      cout <<  "\tBit_parallel_delay_SA (ns): " <<
      (fr->data_array2->delay_SA_R_C) * 1e9 << endl;
      cout <<  "\tBit_parallel_delay_Bitline_MUX (ns): " <<
      (fr->data_array2->delay_BitMux_C) * 1e9 << endl;
      cout <<  "\tBit_parallel_delay_MAX_RC (ns): " <<
      (fr->data_array2->delay_R_C_max) * 1e9 << endl;
      cout <<  "\tBit_parallel_delay_Adder (ns): " <<
      (fr->data_array2->delay_Add) * 1e9 << endl;
      cout <<  "\tBit_parallel_delay_Multiplier (ns): " <<
      (fr->data_array2->delay_Mult) * 1e9 << endl;
      cout <<  "\tBit_parallel_delay_WriteDriver (ns): " <<
      (fr->data_array2->delay_WriteDriver) * 1e9 << endl;
      cout <<  "\tBit_parallel_delay_MAX_AddWdBit_Wordline (ns): " <<
      (fr->data_array2->delay_AWdB_W_max) * 1e9 << endl;
      cout <<  "\tBit_parallel_delay_MAX_MultWdBit_Wordline (ns): " <<
      (fr->data_array2->delay_MWdB_W_max) * 1e9 << endl;
      cout <<  "\tBit_parallel_delay_Final_A_single (ns): " <<
      (fr->data_array2->delay_final_A) * 1e9 << endl;
      cout <<  "\tBit_parallel_delay_Final_M_single (ns): " <<
      (fr->data_array2->delay_final_M) * 1e9 << endl;
      cout <<  "\tBit_parallel_delay_Final_ADD (ns): " <<
      (fr->data_array2->test) * 1e100 << endl;
    ...
    if (g_ip->is_cache && !g_ip->is_main_mem)
    {
      ...
      cout <<  "\tDecoder(ns): " <<
        fr->tag_array2->delay_row_predecode_driver_and_block * 1e9 <<endl;
      
      cout <<  "\twordline delay (ns): " <<
    ...
    cout <<  "\tBitlines data (nJ): " <<
      fr->data_array2->power_bitlines.readOp.dynamic * 1e9 << endl;
    cout <<  "\tSense amplifier energy data (nJ): " <<
    ...
    //bit serial components energy consumption
    cout << endl << endl << "_______Compuatation energy Components:____" << endl << endl;
    cout <<  "\tEnergy per access in compute (nJ): " <<
      fr->data_array2->energy_per_access.computeOp.dynamic * 1e9 << endl;
    cout <<  "\tcompute adder energy (nJ): " <<
      fr->data_array2->energy_per_compute_add.computeOp.dynamic * 1e9 << endl;
    cout <<  "\tcompute mutliplier energy (nJ): " <<
      fr->data_array2->energy_per_compute_mult.computeOp.dynamic * 1e9 << endl;  
    cout <<  "\tEnergy per writeback (nJ): " <<
      fr->data_array2->energy_per_writeback.computeOp.dynamic * 1e9 << endl;
    
      
    

    
    //MAT level components energy
    cout << endl << endl << "_______MAT level components energy compute:____" << endl << endl;
    cout <<  "\tPrecdecoding energy (nJ): " <<
      fr->data_array2->predecoder_energy.computeOp.dynamic * 1e9 << endl;
    cout <<  "\tWordline energy during compute (nJ): " <<
      fr->data_array2->wordlines_energy.computeOp.dynamic * 1e9 << endl;
    cout <<  "\tBitline energy during compute (nJ): " <<
      fr->data_array2->bitlines_energy.computeOp.dynamic * 1e9 << endl;
    cout <<  "\tSenseamp energy during compute (nJ): " <<
      fr->data_array2->sense_amp_energy.computeOp.dynamic * 1e9 << endl;
    cout <<  "\tBitline mux energy during compute (nJ): " <<
      fr->data_array2->bitline_mux_energy.computeOp.dynamic * 1e9 << endl;

    cout << endl << endl << endl << endl;


    //subarray level read components energy
    cout << endl << endl << "_______SUBARRAY level components energy read:____" << endl << endl;
    cout <<  "\tROW Precdecoding energy (fJ): " <<
      fr->data_array2->predecoder_energy_r.readOp.dynamic * 1e15 << endl;
    cout <<  "\tB_MUX Precdecoding energy (fJ): " <<
      fr->data_array2->predecoder_energy_b_mux.readOp.dynamic * 1e15 << endl;
    cout <<  "\tsa_mux_lev_1 Precdecoding energy (fJ): " <<
      fr->data_array2->predecoder_energy_sa_mux_lev_1.readOp.dynamic * 1e15 << endl;
    cout <<  "\tsa_mux_lev_2 Precdecoding energy (fJ): " <<
      fr->data_array2->predecoder_energy_sa_mux_lev_2.readOp.dynamic * 1e15 << endl;
    

    cout <<  "\tWordline energy subarray read (fJ): " <<
      fr->data_array2->decoder_energy_r_sub.readOp.dynamic * 1e15 << endl;
    cout <<  "\tPrecharge BL eq drv subarray energy read(fJ): " <<
      fr->data_array2->precharge_bl_eq_drv_sub.readOp.dynamic * 1e15 << endl;
    cout <<  "\tBitline energy subarray read (fJ): " <<
      fr->data_array2->power_bitline_energy_sub.readOp.dynamic * 1e15 << endl;
    cout <<  "\tSense Amp energy subarray read (fJ): " <<
      fr->data_array2->power_sa_energy_sub.readOp.dynamic * 1e15 << endl;
    
    //subarray level compute components energy
    cout << endl << endl << "_______SUBARRAY level components energy compute:____" << endl << endl;  
    cout <<  "\tWordline energy subarray compute (fJ): " <<
      fr->data_array2->decoder_energy_r_sub.computeOp.dynamic * 1e15 << endl;

    cout <<  "\tPrecharge BL eq drv subarray energy compute(fJ): " <<
      fr->data_array2->precharge_bl_eq_drv_sub.computeOp.dynamic * 1e15 << endl;

    cout <<  "\tBitline energy subarray compute (fJ): " <<
      fr->data_array2->power_bitline_energy_sub.computeOp.dynamic * 1e15 << endl;

    cout <<  "\tSense Amp energy subarray compute (fJ): " <<
      fr->data_array2->power_sa_energy_sub.computeOp.dynamic * 1e15 << endl;

    cout <<  "\tAdder energy subarray compute (fJ): " <<
      fr->data_array2->power_adder_subarray.computeOp.dynamic * 1e15 << endl;

    cout <<  "\tMultiplier energy subarray compute (fJ): " <<
      fr->data_array2->power_multiplier_subarray.computeOp.dynamic * 1e15 << endl;
