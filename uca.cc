...
double UCA::compute_delays(double inrisetime)
{
  ...
  else
  {
    ...
    access_time_write = bank.mat.row_dec->delay + bank.mat.delay_bitline + bank.mat.delay_write_driver;  // row_path

    access_time_writeback = access_time_write - bank.mat.row_dec->delay;  //write_driver delay BEST CASE (Himanshu)
    //Interleaved with decoder

    // access_time_writeback = access_time_write;
    //Worst Case


    access_time_imc=0;
    //double cycle_time_imc=0,cycle_time_imc_add=0;
    if(g_ip->operation_type == 0)
    {
      access_time_imc = MAX(MAX(max_delay_before_row_decoder + delay_inside_mat,  // row_path
            delay_array_to_mat + bank.mat.b_mux_predec->delay + bank.mat.bit_mux_dec->delay + bank.mat.delay_sa),  // col_path
        MAX(delay_array_to_sa_mux_lev_1_decoder,    // sa_mux_lev_1_path
            delay_array_to_sa_mux_lev_2_decoder));
      

      test = bank.mat.delay_write_driver;
      

      // access_time_imc = MAX(bank.mat.row_dec->delay + bank.mat.delay_bitline + bank.mat.delay_sa,  //row_path
      //                       bank.mat.bit_mux_dec->delay + bank.mat.delay_sa);      //col_path
      
      access_time_adder = bank.mat.delay_adder_compute; //add
      access_time_multiplier = bank.mat.delay_multiplier_compute; //multi

      cycle_time_imc_add = MAX(bank.mat.row_dec->delay + bank.mat.delay_bitline + bank.mat.delay_sa,  //row_path
                            bank.mat.bit_mux_dec->delay + bank.mat.delay_sa) +      //col_path
                            MAX(access_time_adder + access_time_writeback,
                            bank.mat.row_dec->delay);   
      
      cycle_time_imc = MAX(bank.mat.row_dec->delay + bank.mat.delay_bitline + bank.mat.delay_sa,  //row_path
                            bank.mat.bit_mux_dec->delay + bank.mat.delay_sa) +      //col_path
                            MAX(access_time_multiplier+ access_time_writeback,
                            bank.mat.row_dec->delay);
                            
      // cycle_time_imc_add = MAX(bank.mat.row_dec->delay + bank.mat.delay_bitline + bank.mat.delay_sa,  //row_path
      //                       bank.mat.bit_mux_dec->delay + bank.mat.delay_sa) +      //col_path
      //                       MAX(access_time_adder+bank.mat.delay_bitline + bank.mat.delay_write_driver,
      //                       bank.mat.row_dec->delay);   
      
      // cycle_time_imc = MAX(bank.mat.row_dec->delay + bank.mat.delay_bitline + bank.mat.delay_sa,  //row_path
      //                       bank.mat.bit_mux_dec->delay + bank.mat.delay_sa) +      //col_path
      //                       MAX(access_time_multiplier+bank.mat.delay_bitline + bank.mat.delay_write_driver,
      //                       bank.mat.row_dec->delay);
    
      
      cycle_time_imc = MAX(cycle_time_imc,cycle_time_imc_add);

      delay_Wordline_R = bank.mat.row_dec->delay;
      delay_Bitline_R = bank.mat.delay_bitline;
      delay_SA_R_C = bank.mat.delay_sa;
      delay_BitMux_C = bank.mat.bit_mux_dec->delay;
      delay_R_C_max = delay_SA_R_C+ MAX(delay_Wordline_R+delay_Bitline_R,
                          delay_BitMux_C);
      delay_Add = access_time_adder;
      delay_Mult = access_time_multiplier;
      delay_WriteDriver = access_time_writeback;
      delay_AWdB_W_max = MAX(delay_Add+access_time_writeback,
                              delay_Wordline_R);
      delay_MWdB_W_max = MAX(delay_Mult+access_time_writeback, 
                              delay_Wordline_R);
      delay_final_A = delay_R_C_max + delay_AWdB_W_max;
      delay_final_M = delay_R_C_max + delay_MWdB_W_max;

      test = bank.mat.delay_write_driver;
      //Himanshu adding precharge delay in each cycle
      cycle_time_imc += 0.3*1e-9;
      cycle_time_imc_add += 0.3*1e-9;
      // bit_serial_add_delay = (g_ip->n_bit + 1) * cycle_time_imc; //n+1 cycles for bs_add
      bit_serial_add_delay = (g_ip->n_bit + 1) * cycle_time_imc_add; //n+1 cycles for bs_add

      bit_serial_multiplier_delay = ((g_ip->n_bit * g_ip->n_bit) + (5 * g_ip->n_bit) + 1)* cycle_time_imc; //n2+5n+1 cycles bs_mult
      //Himanshu Adding predecoding delay once in the end
      //Before row decoder + mat + ....? or only predecoding
      bit_serial_add_delay += bank.mat.b_mux_predec->delay;
      bit_serial_multiplier_delay += bank.mat.b_mux_predec->delay;
    }
    else if (g_ip->operation_type == 1)
    {
      ...
      access_time_adder = bank.mat.delay_adder_compute;
      access_time_multiplier = bank.mat.delay_multiplier_compute;
      access_time_writeback = access_time_write - bank.mat.row_dec->delay;  //write_driver delay
      ...
      delay_SA_R_C = bank.mat.delay_sa;
      delay_BitMux_C = bank.mat.bit_mux_dec->delay;
      delay_R_C_max = MAX(delay_Wordline_R+delay_Bitline_R+delay_SA_R_C,
                          delay_BitMux_C+delay_SA_R_C);
      delay_Add = access_time_adder;
      delay_Mult = access_time_multiplier;
      delay_WriteDriver = access_time_writeback;
      delay_AWdB_W_max = MAX(delay_Add+delay_Bitline_R+delay_WriteDriver,
                              delay_Wordline_R);
      delay_MWdB_W_max = MAX(delay_Mult+delay_Bitline_R+delay_WriteDriver,
                              delay_Wordline_R);
      delay_final_A = delay_R_C_max + delay_AWdB_W_max;
      delay_final_M = delay_R_C_max + delay_MWdB_W_max;
      ...
      // bit_parallel_add_delay = cycle_time_imc;// read+bit_parallel_compute_add+writeback delay
      bit_parallel_add_delay = cycle_time_imc_add;
      
      bit_parallel_multiplier_delay = (g_ip->n_bit + 4) * cycle_time_imc;// read+bit_parallel_compute_mult+writeback delay
      //Himanshu Adding predecoding delay once in the end
      //Before row decoder + mat + ....? or only predecoding
      bit_parallel_add_delay += bank.mat.b_mux_predec->delay;
      bit_parallel_multiplier_delay += bank.mat.b_mux_predec->delay;
    }
