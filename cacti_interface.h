...
class powerDef
{
  public:
    ...
    powerComponents computeOp; //adder_type_step9
    
    powerComponents writebackOp;

    powerDef() : readOp(), writeOp() ,computeOp() { } //adder_type_step10
    void reset() { readOp.reset(); writeOp.reset(); computeOp.reset(); } //adder_type_step11
...
class InputParameter
{
  public:
    ...
    int adder_type;//including adder_type_step2
    int operation_type;//including adder_type_step7
    int sense_amp_type;
...
class mem_array
{
  ...
  double bitline_capacitance;
  double number_of_subarrays_per_mat;
  powerDef single_multiplier_energy;
  powerDef single_adder_energy;
  
  double no_of_columns;//yash no. of columns added to print
  double no_of_rows;//yash no. of rows added to print
  ...
  double compute_adder_delay; //adder_type_step16 yash
  double compute_multiplier_delay;

  //bit_serial and parallel computations delay
  double start_to_sense_delay;
  double write_back_delay;
  double total_nbit_serial_delay_add;
  double total_nbit_parallel_delay_add;
  double total_nbit_serial_delay_multiplier;
  double total_nbit_parallel_delay_multiplier;

//Himanshu
//Delay
  double delay_predec;
  double delay_Wordline_R;
  double delay_Bitline_R;
  double delay_SA_R_C;
  double delay_BitMux_C;
  double delay_R_C_max;
  double delay_Add;
  double delay_Mult;
  double delay_WriteDriver;
  double delay_AWdB_W_max;
  double delay_MWdB_W_max;
  double delay_final_A;
  double delay_final_M;
  double test;
  double delay_precharge;
  //Energy
  powerDef Power_SA_mux_lv1_dec;
  powerDef Power_SA_mux_lv2_dec;

  //bit_serial and parallel computations energy
  powerDef energy_per_access;
  powerDef energy_per_compute_add;
  powerDef energy_per_compute_mult;
  powerDef energy_per_writeback;
  powerDef energy_for_one_compuatation_add;
  powerDef Total_energy_bit_serial_add;
  powerDef energy_for_one_compuatation_multiply;
  powerDef Total_energy_bit_serial_multiplication;

  //parallel
  powerDef Total_energy_bit_parallel_multiplier;
  powerDef Total_energy_bit_parallel_add;

  //parallel computations
  double num_parallel_computes_bs_subarray;
  double num_parallel_computes_bp_subarray;
  double cycle_time_imc;
  double cycle_time_imc_add;

  //bit_serial and parallel computations energy components wise
  powerDef predecoder_energy;
  powerDef wordlines_energy;
  powerDef bitlines_energy;
  powerDef sense_amp_energy;
  powerDef bitline_mux_energy;


  //subarray values
  powerDef predecoder_energy_r;
  powerDef predecoder_energy_b_mux;
  powerDef predecoder_energy_sa_mux_lev_1;
  powerDef predecoder_energy_sa_mux_lev_2;
  powerDef decoder_energy_r_sub;
  powerDef precharge_bl_eq_drv_sub;
  powerDef power_bitline_energy_sub;
  powerDef power_sa_energy_sub;
  powerDef power_adder_subarray;
  powerDef power_multiplier_subarray;

  //Himanshu 
  powerDef power_sa_for_compute;
  powerDef power_bitline_for_compute;
  powerDef power_rowdec_for_compute;
  powerDef power_bitmux_for_compute;
  powerDef power_rowdec_for_writeback;
