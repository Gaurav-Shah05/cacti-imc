...
double Mat::compute_delayadder_CRAM(double inrisetime)
{

double outrisetime ;
outrisetime = compute_adder_delay_CRAM(outrisetime);
return outrisetime;
}

double Mat:: compute_adder_delay_CRAM(double inrisetime)
{
...
double WNAND1_p1    =  145 * g_ip->F_sz_um;
double WNAND1_p2    =  145 * g_ip->F_sz_um;
...
adder_area_circuit += compute_gate_area(NAND, 2, WNAND1_p1, WNAND1_n1,  g_tp.cell_h_def);
...
adder_area_circuit_dff += 2 * compute_gate_area(DFF, 2, WDFF1_p1, WDFF1_n1,  g_tp.cell_h_def) * num_adder_subarray * num_subarrays_per_mat;
...
adder_area_circuit_final = adder_area_circuit * num_adder_subarray * num_subarrays_per_mat ;

// Calculation of Final Leakage Power

double adderlkg_NOR1 = (I_WNOR1_n1) * NOR2_LEAK_STACK_FACTOR * num_adder_subarray * num_subarrays_per_mat;
double adderlkg_NAND1 = (I_WNAND1_n1) * NAND2_LEAK_STACK_FACTOR * num_adder_subarray * num_subarrays_per_mat;
double adderlkg_INV1 = (I_WINV1_n1) * INV_LEAK_STACK_FACTOR * num_adder_subarray * num_subarrays_per_mat;
double adderlkg_NOR2 = (I_WNOR2_n1) * NOR2_LEAK_STACK_FACTOR * num_adder_subarray * num_subarrays_per_mat;
double adderlkg_INV2 = (I_WINV2_n1) * INV_LEAK_STACK_FACTOR * num_adder_subarray * num_subarrays_per_mat;
double adderlkg_DFF1 = (I_WDFF1_n1) * DFF2_LEAK_STACK_FACTOR * num_adder_subarray * num_subarrays_per_mat;
double adderlkg_INV3 = (I_WINV3_n1) * INV_LEAK_STACK_FACTOR * num_adder_subarray * num_subarrays_per_mat;
double adderlkg_INV4 = (I_WINV4_n1) * INV_LEAK_STACK_FACTOR * num_adder_subarray * num_subarrays_per_mat;
double adderlkg_INV5 = (I_WINV5_n1) * INV_LEAK_STACK_FACTOR * num_adder_subarray * num_subarrays_per_mat;
double adderlkg_XOR1 = (I_WXOR1_n1) * XOR2_LEAK_STACK_FACTOR * num_adder_subarray * num_subarrays_per_mat;
double adderlkg_INV6 = (I_WINV6_n1) * INV_LEAK_STACK_FACTOR * num_adder_subarray * num_subarrays_per_mat;
...
inrisetime = this_delay_NOR1/(1.0 - 0.5); 
 ...
 // For NAND1
double C_load_NAND1 = drain_C_(WNAND1_n1, NCH, 2, 1, g_tp.cell_h_def, is_dram)
 + drain_C_(WNAND1_p1, PCH, 1, 1, g_tp.cell_h_def, is_dram)
 + drain_C_(WNAND1_p2, PCH, 1, 1, g_tp.cell_h_def, is_dram)
 + gate_C(WINV2_p1, 0 , is_dram)
 + gate_C(WINV2_n1, 0 , is_dram)
;
double rd_NAND1 = tr_R_on(WINV2_n1, PCH, 1, is_dram) ;
...
inrisetime = this_delay_INV3/(1.0 - 0.5); 
 ...
 // For INV4
double C_load_INV4 = gate_C(WNAND1_n2, 0 , is_dram)
...
;
double rd_INV4 = tr_R_on(WXOR1_n2, NCH, 1, is_dram) ;
...
inrisetime = this_delay_INV6/(1.0 - 0.5); 
    ...
    power_adder.computeOp.dynamic += 0.5 * (C_load_INV6  + C_load_XOR1  + C_load_INV5  + C_load_INV4  + C_load_INV2  + C_load_NOR2  + C_load_INV1  + C_load_NAND1
      + C_load_NOR1 + C_load_DFF1 + C_load_INV3  )* g_tp.peri_global.Vdd * g_tp.peri_global.Vdd * num_subarrays_per_mat ; 

    // power_adder.computeOp.dynamic = 48.3 * 1e-15 * num_adder_subarray * num_subarrays_per_mat;

    //Calculation of Final Leakage Power

    power_adder.computeOp.leakage += (adderlkg_INV6 + adderlkg_XOR1 + adderlkg_INV5 + adderlkg_INV4 + adderlkg_INV3 + adderlkg_DFF1 + adderlkg_INV2 + adderlkg_NOR2 + adderlkg_INV1 + adderlkg_NAND1 + adderlkg_NOR1 ) * g_tp.peri_global.Vdd; 

        adder_area = (adder_area_circuit_final + adder_area_circuit_dff);

        delay_adder_compute =  this_delay_NOR1 + this_delay_NAND1 +  this_delay_INV1 + this_delay_NOR2 + this_delay_INV2 + this_delay_INV3
         + this_delay_DFF1 + this_delay_INV4;

        //delay_adder_compute = 0.411 * 1e-9;
        // cout<<"this_delay_NOR1 :"<<this_delay_NOR1<<endl;
        // cout<<"delay_adder_compute :" <<delay_adder_compute<<endl;
    ...
    delay_adder_carry = this_delay_NOR1 + this_delay_NAND1 +  this_delay_INV1 + this_delay_NOR1 + this_delay_INV2;  //Carry Delay
    // delay_adder_carry = 0.41 * 1e-9;
    delay_adder_sum = this_delay_NOR1 + this_delay_INV3 + this_delay_INV4 + this_delay_XOR1 + this_delay_INV6;     //Sum Delay
    // delay_adder_sum = 0.411 * 1e-9;
    ...
    power_adder.computeOp.dynamic = 0.5 *(C_load_INV6  + C_load_XOR1  + C_load_INV5  + C_load_INV4  + C_load_INV2  + C_load_NOR2  + 
    C_load_INV1  + C_load_NAND1  + C_load_NOR1  )* g_tp.peri_global.Vdd * g_tp.peri_global.Vdd * num_of_parallel_computations*num_subarrays_per_mat ; 
    
    // power_adder.computeOp.dynamic = (g_ip->n_bit ) * 48.3 * 1e-15 * num_of_parallel_computations* num_subarrays_per_mat;


    //Calculation of Final Leakage Power

    power_adder.computeOp.leakage = ( adderlkg_INV6 + adderlkg_XOR1 + adderlkg_INV5 + adderlkg_INV4 + adderlkg_INV2 + adderlkg_NOR2 + adderlkg_INV1 + adderlkg_NAND1 +
     adderlkg_NOR1 ) * g_tp.peri_global.Vdd; 
    }
    ...
    //     power_adder.computeOp.dynamic = 0.5 *(C_load_INV6  + C_load_XOR1  + C_load_INV5  + C_load_INV4  + C_load_INV2  + C_load_NOR2  + 
    //     C_load_INV1  + C_load_NAND1  + C_load_NOR1  )* g_tp.peri_global.Vdd * g_tp.peri_global.Vdd * num_subarrays_per_mat ;
