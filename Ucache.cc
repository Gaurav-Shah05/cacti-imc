    ...
    bool is_main_mem)
{
  ...
  //cout<<"flag_results_populate :"<<flag_results_populate<<endl;
  ...
  else
  {
    ...
    //adder_type_step15 yash
    //ptr_array->compute_adder_delay              = uca->bank.mat.delay_adder_compute; //included at bit_serial and parallel compute
    
    //debug
    ptr_array->bitline_capacitance = uca->bank.mat.subarray.C_bl;
    ptr_array-> number_of_subarrays_per_mat = dyn_p.num_subarrays/dyn_p.num_mats;
    ptr_array-> single_multiplier_energy.computeOp.dynamic = uca->bank.mat.power_multiplier.computeOp.dynamic;
    ptr_array-> single_adder_energy.computeOp.dynamic = uca->bank.mat.power_adder.computeOp.dynamic;

    //ends
    
    //bit_serail and parallel compuatation delay starts
    ptr_array->start_to_sense_delay =uca->access_time_imc;
    ptr_array->compute_adder_delay =uca->access_time_adder;
    ptr_array->compute_multiplier_delay =uca->access_time_multiplier;
    ptr_array->write_back_delay =uca->access_time_writeback;
    ptr_array->total_nbit_serial_delay_add =uca->bit_serial_add_delay;
    ptr_array->total_nbit_parallel_delay_add =uca->bit_parallel_add_delay;


    ptr_array->total_nbit_serial_delay_multiplier=uca->bit_serial_multiplier_delay;
    ptr_array->total_nbit_parallel_delay_multiplier =uca->bit_parallel_multiplier_delay;
    //ends

    //Himanshu
    ptr_array->delay_predec = uca->bank.mat.b_mux_predec->delay;
    ptr_array->delay_Wordline_R = uca->bank.mat.row_dec->delay;
    ptr_array->delay_Bitline_R = uca->bank.mat.delay_bitline;
    ptr_array->delay_SA_R_C = uca->bank.mat.delay_sa;
    ptr_array->delay_BitMux_C = uca->bank.mat.bit_mux_dec->delay;
    ptr_array->delay_R_C_max = uca->delay_R_C_max;
    ptr_array->delay_Add = uca->access_time_adder;
    ptr_array->delay_Mult = uca->delay_Mult;
    ptr_array->delay_WriteDriver = uca->delay_WriteDriver;
    ptr_array->delay_AWdB_W_max = uca->delay_AWdB_W_max;
    ptr_array->delay_MWdB_W_max = uca->delay_MWdB_W_max;
    ptr_array->delay_final_A = uca->delay_final_A;
    ptr_array->delay_final_M = uca->delay_final_M;
    // ptr_array->test = uca->test;
    ptr_array->delay_precharge = 0.3*1e-9;
    
    //bit_serail and parallel compuatation energy starts
    ptr_array-> energy_per_access.computeOp.dynamic = uca->bank.mat.power_access_energy.computeOp.dynamic;
    ptr_array-> energy_per_compute_add.computeOp.dynamic = uca->bank.mat.power_adder_energy.computeOp.dynamic;
    ptr_array-> energy_per_compute_mult.computeOp.dynamic = uca->bank.mat.power_multiplier_energy.computeOp.dynamic;

    ptr_array-> energy_per_writeback.computeOp.dynamic = uca->bank.mat.power_writeback_energy.computeOp.dynamic;
    ptr_array-> energy_for_one_compuatation_add.computeOp.dynamic = uca->bank.mat.Total_add_energy_for_one_computation.computeOp.dynamic;
    ptr_array-> Total_energy_bit_serial_add.computeOp.dynamic = uca->bank.mat.Total_energy_for_bit_serial_add.computeOp.dynamic;
    ptr_array-> energy_for_one_compuatation_multiply.computeOp.dynamic = uca->bank.mat.Total_multiplier_energy_for_one_computation.computeOp.dynamic;
    ptr_array-> Total_energy_bit_serial_multiplication.computeOp.dynamic = uca->bank.mat.Total_energy_for_bit_serial_multiplier.computeOp.dynamic;
    
    //parallel
    ptr_array-> Total_energy_bit_parallel_add.computeOp.dynamic = uca->bank.mat.Total_energy_for_bit_parallel_add.computeOp.dynamic;
    ptr_array-> Total_energy_bit_parallel_multiplier.computeOp.dynamic = uca->bank.mat.Total_energy_for_bit_parallel_multiplier.computeOp.dynamic;
    
    //parallel computations
    //bs
    ptr_array-> num_parallel_computes_bs_subarray = uca->bank.mat.subarray.num_cols / uca->bank.mat.deg_bl_muxing;
    //bp
    ptr_array-> num_parallel_computes_bp_subarray = uca->bank.mat.subarray.num_cols / (uca->bank.mat.deg_bl_muxing*
    g_ip->n_bit);
    
    ptr_array-> cycle_time_imc = uca->cycle_time_imc;
    ptr_array->cycle_time_imc_add = uca->cycle_time_imc_add;

    // cout<<"ptr_array-> cycle_time_imc :"<<ptr_array-> cycle_time_imc<<endl;
    // cout<<"uca->cycle_time_imc :"<<uca->cycle_time_imc<<endl;

    //ends
    
    //component wise energy compute MAT
    ptr_array-> predecoder_energy.computeOp.dynamic = uca->bank.mat.power_predecoder_compute.computeOp.dynamic;
    ptr_array-> wordlines_energy.computeOp.dynamic = uca->bank.mat.power_wordlines_compute.computeOp.dynamic;
    ptr_array-> bitlines_energy.computeOp.dynamic = uca->bank.mat.power_bitlines_compute.computeOp.dynamic;
    ptr_array-> sense_amp_energy.computeOp.dynamic= uca->bank.mat.power_senseamp_compute.computeOp.dynamic;
    ptr_array-> bitline_mux_energy.computeOp.dynamic= uca->bank.mat.power_bitline_mux_compute.computeOp.dynamic;
    //ends
    

    //component wise energy READ subarray
    //step1 predecode
    ptr_array-> predecoder_energy_r.readOp.dynamic = uca->bank.mat.r_predec->power.readOp.dynamic;
    ptr_array-> predecoder_energy_b_mux.readOp.dynamic = uca->bank.mat.b_mux_predec->power.readOp.dynamic;
    ptr_array-> predecoder_energy_sa_mux_lev_1.readOp.dynamic = uca->bank.mat.sa_mux_lev_1_predec->power.readOp.dynamic;
    ptr_array-> predecoder_energy_sa_mux_lev_2.readOp.dynamic = uca->bank.mat.sa_mux_lev_2_predec->power.readOp.dynamic;
    //step2 decode 
    ptr_array-> decoder_energy_r_sub.readOp.dynamic        = uca->bank.mat.power_row_decoders_subarray.readOp.dynamic;
    //step 3 precharge
    ptr_array-> precharge_bl_eq_drv_sub.readOp.dynamic =uca->bank.mat.power_bl_precharge_eq_drv_subarray.readOp.dynamic;
    //step 4 bitline discharge
    ptr_array-> power_bitline_energy_sub.readOp.dynamic = uca->bank.mat.power_bitline_subarray.readOp.dynamic;
    //step 5 sense amp energy
    ptr_array-> power_sa_energy_sub.readOp.dynamic = uca->bank.mat.power_sa.readOp.dynamic;
    //

    //component wise energy compute subarray
    //step2 decode 
    ptr_array-> decoder_energy_r_sub.computeOp.dynamic        = uca->bank.mat.power_row_decoders_subarray.computeOp.dynamic;
    //step 3 precharge
    ptr_array-> precharge_bl_eq_drv_sub.computeOp.dynamic =uca->bank.mat.power_bl_precharge_eq_drv_subarray.computeOp.dynamic;
    //step 4 bitline discharge
    ptr_array-> power_bitline_energy_sub.computeOp.dynamic = uca->bank.mat.power_bitline_subarray.computeOp.dynamic;
    //step 5 sense amp energy
    ptr_array-> power_sa_energy_sub.computeOp.dynamic = uca->bank.mat.power_sa.computeOp.dynamic;
    //step 6 adder enery
    ptr_array-> power_adder_subarray.computeOp.dynamic = uca->bank.mat.power_sa.computeOp.dynamic;
    //step 7 multiplier enery
    ptr_array-> power_multiplier_subarray.computeOp.dynamic = uca->bank.mat.power_sa.computeOp.dynamic;
    //
    ...
    //Himanshu
    ptr_array->test = dyn_p.num_act_mats_hor_dir;
    ptr_array->Power_SA_mux_lv1_dec.computeOp.dynamic = uca->bank.mat.sa_mux_lev_1_dec->power.readOp.dynamic * dyn_p.num_act_mats_hor_dir;
    ptr_array->Power_SA_mux_lv2_dec.computeOp.dynamic = uca->bank.mat.sa_mux_lev_2_dec->power.readOp.dynamic * dyn_p.num_act_mats_hor_dir;
    ptr_array->power_sa_for_compute.computeOp.dynamic = uca->bank.mat.power_sa.computeOp.dynamic;
    ptr_array->power_bitline_for_compute.computeOp.dynamic = uca->bank.mat.power_bitline.computeOp.dynamic;
    ptr_array->power_rowdec_for_compute.computeOp.dynamic = uca->bank.mat.power_row_decoders.computeOp.dynamic;
    ptr_array->power_bitmux_for_compute.computeOp.dynamic = uca->bank.mat.bit_mux_dec->power.readOp.dynamic;
    ptr_array->power_rowdec_for_writeback.writebackOp.dynamic = uca->bank.mat.power_row_decoders.writebackOp.dynamic;
    ...
    //yash no. of columns and rows are added to print 
    ptr_array->no_of_columns = uca->dp.num_c_subarray;
    ptr_array->no_of_rows = uca->dp.num_r_subarray;
...
{
  bool   is_dram  = false;
  //cout<<"g_ip->is_cache :"<<g_ip->is_cache<<endl;
  //cout<<"g_ip->is_main_mem :"<<g_ip->is_main_mem<<endl;
  //exit(0);
  int    pure_ram = (!g_ip->is_cache || g_ip->is_main_mem);
  //cout<<"pure_ram :"<<pure_ram<<endl;
