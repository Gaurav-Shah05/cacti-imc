...
Mat::Mat(const DynamicParameter & dyn_p)
 :dp(dyn_p),
  ...
  adder_area_circuit(0),multiplier_area(0), delay_write_driver(0)
{
...
double Mat::compute_delays(double inrisetime)
{
  ...
  double outrisetime_adder=0,outrisetime_multiplier=0;
  

 
  //adder_type_step14 yash
  if  (g_ip->adder_type == 2)
{
  outrisetime_adder = compute_delayadder_BLADE(outrisetime);
}
  if  (g_ip->adder_type == 0)
{
  outrisetime_adder = compute_delayadder_CRAM(outrisetime);
  }
  if  (g_ip->adder_type == 4)
{
  outrisetime_adder = compute_delayadder_DAC(outrisetime);
  }
  if  (g_ip->adder_type == 3)
{
  outrisetime_adder = compute_delayadder_ISCAS(outrisetime);
  }
  if  (g_ip->adder_type == 1)
{
  outrisetime_adder = compute_delayadder_new(outrisetime);
  }
...
if (g_ip->operation_type == 0)
  ...
  if  (g_ip->adder_type == 2)
{
  outrisetime_multiplier = compute_delaymultiplier_serial_BLADE(outrisetime);
  }
  if  (g_ip->adder_type == 0)
{
  outrisetime_multiplier = compute_delaymultiplier_serial_CRAM(outrisetime);
  }
  if  (g_ip->adder_type == 4)
{
  outrisetime_multiplier = compute_delaymultiplier_serial_DAC(outrisetime);
  }
  if  (g_ip->adder_type == 3)
{
  outrisetime_multiplier = compute_delaymultiplier_serial_ISCAS(outrisetime);
  }
  if  (g_ip->adder_type == 1)
{
  outrisetime_multiplier = compute_delaymultiplier_serial_new(outrisetime);
  }

 outrisetime = MAX(outrisetime_adder,outrisetime_multiplier);

 } 
 else if(g_ip->operation_type == 1)
 {
   outrisetime_multiplier = compute_delaymultiplier_parallel(outrisetime);
   outrisetime = MAX(outrisetime_adder,outrisetime_multiplier);
 }
  ...
  outrisetime = subarray_out_wire->signal_rise_time();//?? yash
...
double Mat::compute_bitline_delay(double inrisetime)
{
  ...
  double bitline_energy_factor;
  if (g_ip->F_sz_nm == 28)
  {
    bitline_energy_factor= 1.589;
  }
  else if (g_ip->F_sz_nm == 65)
  {
    bitline_energy_factor= 1.136;
  }
  else
  {
    bitline_energy_factor=1;
  }
  if (is_dram == true)
  {
  ...
  else
  {
    ...
    if (deg_bl_muxing > 1)
    {
      ...
      dynRdEnergy += (2 * C_drain_sense_amp_iso + C_sense_amp_latch +  C_drain_sense_amp_mux) * 
        2 * dp.V_b_sense * g_tp.sram_cell.Vdd * (subarray.num_cols * num_subarrays_per_mat / deg_bl_muxing) *  bitline_energy_factor;
    ...
    else
    {
      ...
      dynRdEnergy += (C_bl + 2 * C_drain_sense_amp_iso + C_sense_amp_latch + C_drain_sense_amp_mux) *
        2 * dp.V_b_sense * g_tp.sram_cell.Vdd * subarray.num_cols * num_subarrays_per_mat * bitline_energy_factor;
      dynWriteEnergy += (((subarray.num_cols * num_subarrays_per_mat / deg_bl_muxing) / deg_senseamp_muxing) *
  ...
  if (dp.is_tag == false || is_fa == false)
  {
    ...
    power_bitline.computeOp.dynamic = 2 * dynRdEnergy;//yash bitline energy in compute and read cycle is twice
    power_bitline.writeOp.dynamic = dynWriteEnergy;
     
    power_bitline_subarray.readOp.dynamic = (dynRdEnergy)/(num_subarrays_per_mat);
    power_bitline_subarray.computeOp.dynamic = (2 * dynRdEnergy)/(num_subarrays_per_mat);
  }
...
double Mat::compute_sa_delay(double inrisetime)
{
  ...
  //yash included sense_amp delay
  if(g_ip->F_sz_um==0.028)
  {
    if(g_ip->sense_amp_type == 0) //conv
    {
      delay_sa=0.0237*1e-9;
    }
    else if(g_ip->sense_amp_type == 1) //mrsa
    {
      delay_sa=0.039*1e-9;
    }
    else if(g_ip->sense_amp_type == 2) //cesa
    {
      delay_sa=0.025*1e-9;
    }
    else if(g_ip->sense_amp_type == 3) //original cacti
    {
      delay_sa = tau * log(g_tp.peri_global.Vdd / dp.V_b_sense);
    }
    else
  ...
  else if (g_ip->F_sz_um==0.065)
  {
    if(g_ip->sense_amp_type == 0) //conv
    {
      delay_sa=0.1855*1e-9;
    }
    else if(g_ip->sense_amp_type == 1) //mrsa
    {
      delay_sa=0.2757*1e-9;
      //delay_sa = tau * log(g_tp.peri_global.Vdd / dp.V_b_sense);
    }
    else if(g_ip->sense_amp_type == 2) //cesa
    {
  ...
  power_sa.computeOp.dynamic = 2 * power_sa.readOp.dynamic;

  power_sa.readOp.leakage = lkgIdle * g_tp.peri_global.Vdd;

  power_sa_subarray.readOp.dynamic    = power_sa.readOp.dynamic/num_subarrays_per_mat ; //yash
  power_sa_subarray.computeOp.dynamic = 2 * power_sa.readOp.dynamic/num_subarrays_per_mat ; //yash
...
void Mat::compute_power_energy()
{
  ...
  //two sense_amplifiers used but same predec logic discuss?
  // power.computeOp.dynamic += r_predec->power.readOp.dynamic +  //yash
  //                         b_mux_predec->power.readOp.dynamic +  
  //                         sa_mux_lev_1_predec->power.readOp.dynamic + 
  //                         sa_mux_lev_2_predec->power.readOp.dynamic;

  //power.computeOp.dynamic = 0;//testing purpose not sure

  power.writebackOp.dynamic  = 0;
  // power.writebackOp.dynamic = r_predec->power.readOp.dynamic + //yash
  //                         b_mux_predec->power.readOp.dynamic;
  ...
  //yash writeback defined
  power_bl_precharge_eq_drv.writebackOp.dynamic = bl_precharge_eq_drv->power.readOp.dynamic;
  power_row_decoders.writebackOp.dynamic        = row_dec->power.readOp.dynamic;
  
  //yash compute defined 
  power_bl_precharge_eq_drv.computeOp.dynamic = bl_precharge_eq_drv->power.readOp.dynamic; 
  power_row_decoders.computeOp.dynamic        = row_dec->power.readOp.dynamic + row_dec->power.readOp.dynamic ; //two wordlines are activated during compute operation

  //subarray values
  power_bl_precharge_eq_drv_subarray.readOp.dynamic = bl_precharge_eq_drv->power.readOp.dynamic;
  power_row_decoders_subarray.readOp.dynamic        = row_dec->power.readOp.dynamic;

  power_bl_precharge_eq_drv_subarray.computeOp.dynamic = bl_precharge_eq_drv->power.readOp.dynamic;
  power_row_decoders_subarray.computeOp.dynamic        = row_dec->power.readOp.dynamic + row_dec->power.readOp.dynamic;

  power_adder_subarray.computeOp.dynamic                        = (power_adder.computeOp.dynamic) /num_subarrays_per_mat;
  power_multiplier_subarray.computeOp.dynamic                        = (power_multiplier.computeOp.dynamic) /num_subarrays_per_mat;
  ...
  if (is_fa == false)
  {
    ...
    //yash writeback defined
    power_bl_precharge_eq_drv.writebackOp.dynamic *= num_subarrays_per_mat;
    power_row_decoders.writebackOp.dynamic        *= num_subarrays_per_mat;
    
    //yash compute defined
    power_bl_precharge_eq_drv.computeOp.dynamic *= num_subarrays_per_mat;
    power_row_decoders.computeOp.dynamic        *= num_subarrays_per_mat;
  ...
  else if (dp.is_tag == true)
  {
    ...
    //yash writeback defined
    power_row_decoders.writebackOp.dynamic         = power_fa_decoder.writebackOp.dynamic;
     
    //yash compute defined
    power_row_decoders.computeOp.dynamic         = 2 * power_fa_decoder.computeOp.dynamic; //two wordlines are activated during compute operation


  }
  
  // cout<<"Ye hai Test aur iska koi matlab nahi ABOVE IF, power access compute: "<<power.computeOp.dynamic*1e9<<endl;
  if (is_fa == false || dp.is_tag == false)
  {
    ...
    power.readOp.dynamic += power_bl_precharge_eq_drv.readOp.dynamic +
                            ...
                            power_subarray_out_drv.readOp.dynamic; //final read out path delay
    
    // cout<<"Ye hai Test aur iska koi matlab nahi INSIDE IF, power access compute: "<<power.computeOp.dynamic*1e9<<endl;
    // cout<<"Ye hai Test aur iska koi matlab nahi INSIDE IF, power precharge: "<<power_bl_precharge_eq_drv.readOp.dynamic*1e9<<endl;
    // cout<<"Ye hai Test aur iska koi matlab nahi INSIDE IF, power sa: "<<power_sa.computeOp.dynamic*1e9<<endl;
    // cout<<"Ye hai Test aur iska koi matlab nahi INSIDE IF, power bitline: "<<power_bitline.computeOp.dynamic*1e9<<endl;

    //yash compute defined
    power.computeOp.dynamic += power_bl_precharge_eq_drv.readOp.dynamic + //this is for total number of subarrays in a mat
                            power_sa.computeOp.dynamic +  //this is for total number of subarrays in a mat
                            power_bitline.computeOp.dynamic;//this is for total number of subarrays in a mat (twice the read energy)
                            //power_subarray_out_drv.readOp.dynamic; //power_subarray_out_drv should we define??
    
    
    
  }

  // cout<<"Ye hai Test aur iska koi matlab nahi BELOW IF, power row dec: "<<power_row_decoders.computeOp.dynamic*1e9<<endl;
  // cout<<"Ye hai Test aur iska koi matlab nahi BELOW IF, power bitmux: "<<bit_mux_dec->power.readOp.dynamic*1e9<<endl;
  // cout<<"Ye hai Test aur iska koi matlab nahi BELOW IF, power sa mux 1: "<<sa_mux_lev_1_dec->power.readOp.dynamic*1e9<<endl;
  // cout<<"Ye hai Test aur iska koi matlab nahi BELOW IF, power sa mux 2: "<<sa_mux_lev_2_dec->power.readOp.dynamic*1e9<<endl;
  // cout<<"Ye hai Test aur iska koi matlab nahi BELOW IF, power access compute: "<<power.computeOp.dynamic*1e9<<endl;
  ...
  //yash compute defined
  power.computeOp.dynamic += power_row_decoders.computeOp.dynamic + //this is for total number of subarrays in a mat
                          bit_mux_dec->power.readOp.dynamic + //this is for one subarray
                          sa_mux_lev_1_dec->power.readOp.dynamic + //this is for one subarray
                          sa_mux_lev_2_dec->power.readOp.dynamic ; //this is for one subarray
                          //power_comparator.computeOp.dynamic; //tag_comparator not included in imc

  // cout<<"Ye hai Test aur iska koi matlab nahi BELOW BELOW IF, power access compute: "<<power.computeOp.dynamic*1e9<<endl;

  
  //yash writeback defined
  power.writebackOp.dynamic += power_row_decoders.writebackOp.dynamic +
                          bit_mux_dec->power.readOp.dynamic;

  
  
  //Individual Component energy at MAT level
  power_predecoder_compute.computeOp.dynamic = 0;  //predecoding energy_compute
  power_wordlines_compute.computeOp.dynamic = 0; //decoding energy_compute
  power_precharge_bitlines_compute.computeOp.dynamic = 0; //precharge_driver+bitline energy
  power_bitlines_compute.computeOp.dynamic = 0;
  power_senseamp_compute.computeOp.dynamic = 0;  //sense_amp energy_compute
  power_bitline_mux_compute.computeOp.dynamic = 0; //bitline_mux_decoding energy

  power_decoder_wb.writebackOp.dynamic = 0; //predecoding energy_write
  power_wordlines_wb.writebackOp.dynamic = 0; //decoding energy_write
  power_precharge_bitlines_wb.writebackOp.dynamic = 0; //precharge_driver+bitline energy
  power_senseamp_wb.writebackOp.dynamic = 0; //sense_amp energy_write
  power_bitline_mux_wb.writebackOp.dynamic = 0;


  power_predecoder_compute.computeOp.dynamic = r_predec->power.readOp.dynamic ; //Predecoding Logic  Energy
  power_wordlines_compute.computeOp.dynamic = row_dec->power.readOp.dynamic + row_dec->power.readOp.dynamic;  //Decoder Logic
  power_precharge_bitlines_compute.computeOp.dynamic = power_bl_precharge_eq_drv.readOp.dynamic;
  power_bitlines_compute.computeOp.dynamic = power_bitline.readOp.dynamic;
  power_senseamp_compute.computeOp.dynamic = power_sa.readOp.dynamic  + power_sa.readOp.dynamic;
  power_bitline_mux_compute.computeOp.dynamic =  bit_mux_dec->power.readOp.dynamic;

  power_decoder_wb.computeOp.dynamic = 0 ; //Predecoding Logic  Energy  //As we are writing in same sub-array
  power_wordlines_wb.computeOp.dynamic = row_dec->power.readOp.dynamic ;  //Decoder Logic
  power_precharge_bitlines_wb.computeOp.dynamic = power_bitline.readOp.dynamic;
  power_senseamp_wb.computeOp.dynamic = 0;
  power_bitline_mux_wb.computeOp.dynamic =  0;
  ...
  if(g_ip->operation_type == 0)
  { 
      //read_energy for one access
      power_access_energy.computeOp.dynamic = power.computeOp.dynamic;
      power_adder_energy.computeOp.dynamic = power_adder.computeOp.dynamic; //this is for total number of subarrays in a mat
      power_multiplier_energy.computeOp.dynamic = power_multiplier.computeOp.dynamic; //this is for total number of subarrays in a mat
      power_writeback_energy.computeOp.dynamic = power.writebackOp.dynamic;

      Total_add_energy_for_one_computation.computeOp.dynamic = power_access_energy.computeOp.dynamic +
                                                         power_adder_energy.computeOp.dynamic+
                                                         power_writeback_energy.computeOp.dynamic; //add predecoding energy for one cycle

      Total_energy_for_bit_serial_add.computeOp.dynamic = (g_ip->n_bit + 1) * Total_add_energy_for_one_computation.computeOp.dynamic;
      
      Total_multiplier_energy_for_one_computation.computeOp.dynamic = power_access_energy.computeOp.dynamic +
                                                         power_multiplier_energy.computeOp.dynamic+
                                                         power_writeback_energy.computeOp.dynamic;
                                                      
       Total_energy_for_bit_serial_multiplier.computeOp.dynamic = ((g_ip->n_bit * g_ip->n_bit) + (5 * g_ip->n_bit) + 1)*
                                                              Total_multiplier_energy_for_one_computation.computeOp.dynamic;
  ...
  else if(g_ip->operation_type == 1)
  {
    ...
    power_multiplier_energy.computeOp.dynamic = power_multiplier.computeOp.dynamic;

    power_writeback_energy.computeOp.dynamic = power.writebackOp.dynamic;


    Total_energy_for_bit_parallel_add.computeOp.dynamic = power_access_energy.computeOp.dynamic +
                                                         power_adder_energy.computeOp.dynamic+
                                                         power_writeback_energy.computeOp.dynamic;
    
    Total_energy_for_bit_parallel_multiplier.computeOp.dynamic = (g_ip->n_bit + 4) *(power_access_energy.computeOp.dynamic +
                                                         power_multiplier_energy.computeOp.dynamic+
                                                         power_writeback_energy.computeOp.dynamic);
  }
  //computation energy
  // power_subarray_in_compute.computeOp.dynamic= power_bl_precharge_eq_drv.readOp.dynamic +
  //                           power_sa.readOp.dynamic + power_sa.readOp.dynamic + 
  //                           power_bitline.readOp.dynamic +
  //                           power_subarray_out_drv.readOp.dynamic; //power_subarray_out_drv??
